...
Four ways the incremental index accepted a formula it could not keep. One was
reported; the other three came out of generating valid programs at random and
running every one under `Machine(audit=True)`. **Three of the four returned a
wrong answer in silence**, which is worse than the one that raised.
...
Each now declines the index rather than guessing. See [honest
limits](/docs/limits) for the five shapes and why the fold cannot see them.
